...
export function summarizeInteractions(interactions: Interaction[], sessionStart: number): string {
  ...
  const lines: string[] = [
    '## Session Summary',
    '',
    '| # | Type | Status | Started | Δ Time | Duration | Description |',
    '|---|------|--------|---------|--------|----------|-------------|',
  ];
  ...
  interactions.forEach((interaction, index) => {
    ...
    const duration =
      interaction.duration !== undefined ? formatDuration(interaction.duration) : '—';
    const description = interaction.description.replace(/\n/g, ' ');
    lines.push(
      `| ${index + 1} | ${interaction.type} | ${status} | ${startTime} | ${delta} | ${duration} | ${description} |`
    );
  });
