...
import { ContentCapture, CaptureError, CaptureErrorType, type CaptureOptions } from "../ContentCapture.js";
import type { IDOMAnalyzer, IContentSanitizer, IMediaCapture, IReliableSelectionCapture, IContentProcessor } from "../ContentCapture.js";
...
function createDeps() {
  const dom: IDOMAnalyzer = {
    ...
    getSelectionContext: (b: number, a: number) => "before [Selected text] after",
  };

  const sanitizer: IContentSanitizer = {
    sanitize: (content: string) => ({ sanitizedContent: content.replace(/\S+@\S+/, "[EMAIL_REDACTED]"), redactionCount: /@/.test(content) ? 1 : 0, detectedPII: /@/.test(content) ? [{ type: "EMAIL" }] as any : [] })
  };

  const media: IMediaCapture = {
    captureAllMedia: async () => ({ images: [{ metadata: {} as any, dataUrl: "data:image/png;base64,abc", capturedAt: Date.now(), thumbnail: "data:image/png;base64,thumb" } as any], audios: [], videos: [], totalSize: 1024, capturedAt: Date.now() })
  };
...
describe("ContentCapture - mode routing and metadata", () => {
  it("routes by mode and returns metadata for note", async () => {
    ...
    const options: CaptureOptions = { mode: "note", pocketId: "p1", sanitize: false, noteText: "hello" };
...
describe("ContentCapture - selection mode", () => {
  it("captures selection with preview and sanitization off", async () => {
    ...
    const withPreview = await cc.captureWithPreview({ mode: "selection", pocketId: "p1", sanitize: false });
  ...
  it("throws SELECTION_EMPTY when no selection", async () => {
    const deps = makeDeps({ dom: { ...createDeps().dom, extractSelection: () => null } as any });
...
describe("ContentCapture - sanitization toggle", () => {
  it("applies sanitizer when enabled", async () => {
    ...
    deps.dom.extractSelection = () => ({ content: "Contact me test@example.com", wordCount: 3, characterCount: 26, paragraphs: [], headings: [], links: [], images: [], lists: [], tables: [] });
