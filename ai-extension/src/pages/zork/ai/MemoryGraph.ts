...
export class MemoryGraph {
  ...
  getPlayerChoicePattern(): string {
    ...
    const aggressive = choices.filter(
      (c) => c.includes("attack") || c.includes("fight") || c.includes("kill"),
    ).length;

    const peaceful = choices.filter(
      (c) => c.includes("talk") || c.includes("help") || c.includes("give"),
    ).length;

    const cautious = choices.filter(
      (c) => c.includes("examine") || c.includes("look") || c.includes("wait"),
    ).length;
