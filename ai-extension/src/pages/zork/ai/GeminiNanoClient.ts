  ...
  async initialize(): Promise<boolean> {
    try {
      ...
      if (!('LanguageModel' in globalThis)) {
        console.error('Chrome Prompt API not available. Enable it in chrome://flags/#prompt-api-for-gemini-nano');
        return false;
      }
      ...
      const availabilityStatus = typeof availability === 'string' 
        ? availability 
      ...
      if (availabilityStatus === 'readily' || availabilityStatus === 'available') {
        ...
        if (!this.promptSession) {
          console.error('[Zork] Failed to create Prompt API session - session is null');
          return false;
        }
        ...
        console.error('[Zork] Gemini Nano is not available on this device. Status:', availabilityStatus);
        return false;
      }
...
Just return the text response, no JSON.`;
  ...
  async generateNPCDialogue(npc: any, playerInput: string, history: string[]): Promise<string> {
    if (!this.promptSession) {
      throw new Error('Prompt API not available');
    }
...
Just return the dialogue, no JSON.`;
  ...
  private getFallbackLocation(): any {
    return {
      ...
      description: 'You find yourself in a dimly lit chamber. The walls are covered in ancient runes that seem to shimmer in the faint light.',
      exits: ['north', 'south', 'east', 'west'],
      items: [],
      npcs: [],
