...
export interface StoryBeat {
  type: 'discovery' | 'conflict' | 'mystery' | 'revelation' | 'twist' | 'calm' | 'climax';
...
export interface NarrativeMomentum {
  ...
  emotionalTone: 'wonder' | 'dread' | 'excitement' | 'melancholy' | 'triumph' | 'confusion';
}
...
export class NarrativeDirector {
  ...
  constructor(initialThemes: string[] = []) {
    this.narrativeThemes = initialThemes.length > 0 
      ? initialThemes 
  ...
  private generateRandomThemes(): string[] {
    const themePool = [
      'ancient_prophecy', 'forgotten_civilization', 'reality_manipulation',
      'time_paradox', 'cosmic_horror', 'hidden_conspiracy', 'magical_corruption',
      'technological_singularity', 'dimensional_breach', 'cursed_artifact',
      'lost_memories', 'parallel_worlds', 'sentient_dungeon', 'dream_logic'
    ];
  ...
  private updateMomentum(beat: StoryBeat): void {
    ...
    switch (beat.type) {
      case 'conflict':
        ...
        this.momentum.dangerLevel = Math.min(100, this.momentum.dangerLevel + 10);
        break;
      case 'mystery':
        this.momentum.mysteryLevel = Math.min(100, this.momentum.mysteryLevel + 20);
        this.momentum.tension = Math.min(100, this.momentum.tension + 5);
        break;
      case 'revelation':
        this.momentum.mysteryLevel = Math.max(0, this.momentum.mysteryLevel - 15);
      ...
      case 'climax':
        ...
        this.momentum.dangerLevel = Math.min(100, this.momentum.dangerLevel + 20);
        break;
    }
  ...
  getReadyForeshadowing(): string[] {
    ...
    this.foreshadowingQueue = this.foreshadowingQueue.filter(f => f.triggersIn > 0);
