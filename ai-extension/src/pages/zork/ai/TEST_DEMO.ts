...
function demoNarrativeDirector() {
  ...
  for (let i = 0; i < 10; i++) {
    ...
    if (beatCheck.should) {
      console.log(`Move ${i + 1}: STORY BEAT - ${beatCheck.type.toUpperCase()}`);
      director.recordBeat({
        type: beatCheck.type,
        intensity: 7,
        timestamp: Date.now()
...
function demoVariationEngine() {
  ...
  for (let i = 1; i <= 5; i++) {
    ...
    console.log(variation.generateVariationPrompt(
...
function demoMemoryGraph() {
  ...
  memory.addNode({
    id: 'forest_entrance',
    type: 'location',
    data: { name: 'Forest Entrance', description: 'A dark path leads into the woods' },
    timestamp: Date.now(),
    connections: []
  });
  ...
  memory.addNode({
    id: 'ancient_ruins',
    type: 'location',
    data: { name: 'Ancient Ruins', description: 'Crumbling stones tell forgotten stories' },
    timestamp: Date.now(),
    connections: []
  });
  ...
  console.log('After helping wizard:', memory.getNPCRelationshipDescription('mysterious_wizard'));
  
  memory.updateNPCRelationship('mysterious_wizard', 30);
  console.log('After more help:', memory.getNPCRelationshipDescription('mysterious_wizard'));
  
  memory.updateNPCRelationship('mysterious_wizard', -60);
  console.log('After betrayal:', memory.getNPCRelationshipDescription('mysterious_wizard'));
  ...
  console.log('Active consequences at move 5:', memory.getActiveConsequences(5));
  console.log('Active consequences at move 10:', memory.getActiveConsequences(10));
...
function demoDiscoverySystem() {
  ...
  const commands = ['xyzzy', 'plugh', 'frotz', 'look at the fourth wall', 'ask about the AI'];
  ...
  for (const command of commands) {
    ...
    try {
      ...
      if (response.metadata.discoveredSecret) {
        console.log(`🎉 Secret discovered: ${response.metadata.discoveredSecret}\n`);
      }
    } catch (error) {
