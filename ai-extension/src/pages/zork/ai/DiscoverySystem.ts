...
export interface Secret {
  id: string;
  type: 'hidden_command' | 'easter_egg' | 'lore_fragment' | 'hidden_mechanic' | 'meta_secret';
  trigger: string;
  revealed: boolean;
  description: string;
  reward: string;
...
export class DiscoverySystem {
  ...
  checkForSecret(input: string): Secret | null {
    ...
    for (const secret of this.secrets) {
      if (!secret.revealed && normalized.includes(secret.trigger.toLowerCase())) {
  ...
  private determineRarity(type: Secret['type']): Discovery['rarity'] {
    switch (type) {
      case 'hidden_command': return 'uncommon';
      case 'easter_egg': return 'rare';
      case 'lore_fragment': return 'rare';
      case 'hidden_mechanic': return 'legendary';
      case 'meta_secret': return 'mythic';
  ...
  private checkForMechanicUnlock(action: string): void {
    ...
    if (lower.includes('combine') || lower.includes('mix') || lower.includes('merge')) {
  ...
  private generateHint(secret: Secret): string {
    ...
    const secretHints = hints[secret.id] || ['Something hidden awaits discovery'];
    return secretHints[Math.floor(Math.random() * secretHints.length)] || 'Something hidden awaits discovery';
  }
