...
export class SuspenseManager {
  ...
  generateSuspensePrompt(): string {
    ...
    const oldestElement = activeElements.sort(
      (a, b) => a.planted - b.planted,
    )[0];
    ...
    if (this.unresolvedMysteries.size > 0) {
      ...
      Array.from(this.unresolvedMysteries.keys())
        .slice(0, 3)
        .forEach((m) => {
          prompt += `- ${m}\n`;
        });
    ...
    if (oldestElement && Date.now() - oldestElement.planted > 300000) {
      // 5 minutes
  ...
  shouldRevealTwist(condition: string): {
    should: boolean;
    twist: string | null;
  } {
    ...
    if (availableTwist && Math.random() < 0.3) {
      // 30% chance when condition met
  ...
  escalateThreat(
    type: "ambient" | "immediate" | "existential",
    amount: number,
  ): void {
  ...
  generateTensionModifier(): string {
    const total =
      this.threatLevel.ambient +
      this.threatLevel.immediate +
      this.threatLevel.existential;
  ...
  generatePartialInformation(): string {
    ...
    return (
      partials[Math.floor(Math.random() * partials.length)] ||
      "You sense something, but can't quite grasp it"
    );
  }
