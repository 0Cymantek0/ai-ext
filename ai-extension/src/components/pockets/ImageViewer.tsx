...
export function ImageViewer({
  ...
  const getImageMetadata = () => {
    ...
    if (content.metadata.dimensions) {
      metadata["Dimensions"] =
        `${content.metadata.dimensions.width} × ${content.metadata.dimensions.height}px`;
    }
  ...
  return (
    <>
      {/* Backdrop */}
      <div className="fixed inset-0 bg-zinc-950/95 z-50" onClick={onClose} />
                    ...
                    <div className="space-y-2">
                      {Object.entries(imageMetadata).map(([key, value]) => (
                        <div key={key} className="flex justify-between gap-2">
                          ...
                          <dd className="text-xs text-zinc-200 text-right break-words">
                            {value}
                          </dd>
                        </div>
                      ))}
