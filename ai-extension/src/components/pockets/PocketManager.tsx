...
export const PocketManager = React.forwardRef<
  PocketManagerRef,
  PocketManagerProps
>(
  (
    ...
    const [editingPocket, setEditingPocket] = React.useState<PocketData | null>(
      null,
    );
    const [isLoading, setIsLoading] = React.useState(true);
    const [selectedPocket, setSelectedPocket] =
      React.useState<PocketData | null>(null);
    ...
    React.useEffect(() => {
      const messageListener = (message: any) => {
        if (
          message.kind === "CONTENT_CREATED" ||
          message.kind === "CONTENT_UPDATED"
        ) {
          console.log(
            "PocketManager: Content change detected, reloading pockets",
          );
          loadPockets();
        }
      };
    ...
    const handleCreatePocket = async (
      pocketData: Omit<
        PocketData,
        "id" | "createdAt" | "updatedAt" | "contentIds"
      >,
    ) => {
      try {
        const response = await chrome.runtime.sendMessage({
    ...
    const handleSharePocket = (pocket: PocketData) => {
      ...
      navigator.clipboard.writeText(shareText).then(
        () => {
          alert(
            `"${pocket.name}" copied to clipboard! You can share this with others.`,
          );
        },
        (err) => {
    ...
    React.useImperativeHandle(ref, () => ({
      ...
      openExportImport: (_mode?: "export" | "import") =>
          ...
          {isLoading ? (
            ...
            >
              {filteredPockets.map((pocket) => {
                const pocketIndexingInfo =
                  indexingStatus.getPocketIndexingStatus(pocket.contentIds);
                return (
                  <PocketCard
                    key={pocket.id}
                    pocket={pocket}
                      ...
                      pocketIndexingInfo.totalContent > 0 ? (
                        <PocketIndexingStatus
                          ...
                          onRetry={() => {
                            ...
                            pocketIndexingInfo.failedContentIds.forEach(
                              (contentId) => {
