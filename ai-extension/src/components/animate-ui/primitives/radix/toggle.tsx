...
function Toggle({
  ...
  const [isPressed, setIsPressed] = useControlledState<boolean>({
    value: pressed ?? defaultPressed ?? false,
    defaultValue: defaultPressed ?? false,
    onChange: (next) => onPressedChange?.(next),
  });

  return (
    <ToggleProvider value={{ isPressed, setIsPressed, disabled: !!disabled }}>
