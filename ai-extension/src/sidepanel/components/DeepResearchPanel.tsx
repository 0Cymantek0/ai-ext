...
export interface DeepResearchPanelProps {
  ...
  /** Callback to open the linked research pocket. */
  onOpenPocket: (pocketId: string) => void;
}

/**
 * DeepResearchPanel encapsulates all deep-research workflow UI:
 * topic/goal inputs, launch controls, run status, research state display
 * (pocket, evidence, active question, synthesis, gaps), todo list, and timeline.
 *
...
export function DeepResearchPanel({
  ...
  const handleLaunch = React.useCallback(() => {
    ...
    if (!topic || !goal || isLaunching) {
      return;
    }

    setIsLaunching(true);
    onLaunch(topic, goal);
    // Do NOT clear inputs — user may want to refine and re-launch
    setTimeout(() => setIsLaunching(false), 300);
  }, [topicInput, goalInput, isLaunching, onLaunch]);
    ...
    <div className="flex items-start justify-between gap-3">
      <div>
        ...
        <p className="text-xs text-muted-foreground">
          Launches a canonical research run with subquestions, synthesis, and
  ...
  return (
    <AgentPanelLayout header={header}>
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 text-xs text-muted-foreground">
        ...
        >
          {isLaunching ? "Launching..." : "Launch deep research"}
        </Button>
      </div>
      ...
      {hasActiveRun && (
        <div className="rounded-xl border border-border/60 bg-muted/30 p-3">
          ...
          {(panelState?.pocketId ||
            typeof panelState?.evidenceCount === "number") && (
            <div className="mt-3 grid gap-3 md:grid-cols-2">
              {panelState?.pocketId && (
                <div>
                  <p className="text-[11px] uppercase tracking-wide text-muted-foreground">
                  ...
                  <div className="mt-1 flex items-center gap-2">
                    ...
                    <Button
                      ...
                      variant="outline"
                      size="sm"
                      onClick={() => onOpenPocket(panelState.pocketId!)}
                    >
                      Open linked pocket
                    </Button>
                  </div>
              ...
              {typeof panelState?.evidenceCount === "number" && (
                <div>
                  ...
                  <p className="mt-1 text-sm text-foreground">
                    {panelState.evidenceCount} evidence item
                    {panelState.evidenceCount === 1 ? "" : "s"} captured
                  </p>
                </div>
              )}
            </div>
          ...
          {panelState?.activeQuestion && (
            <div className="mt-3">
              <p className="text-[11px] uppercase tracking-wide text-muted-foreground">
                Active question
              </p>
              <p className="mt-1 text-sm text-foreground">
                {panelState.activeQuestion.question}
              </p>
            </div>
          )}

          {panelState?.latestSynthesis && (
            <div className="mt-3">
              <p className="text-[11px] uppercase tracking-wide text-muted-foreground">
                Latest synthesis
              </p>
          ...
          {panelState?.openGaps && panelState.openGaps.length > 0 && (
            <div className="mt-3">
              <p className="text-[11px] uppercase tracking-wide text-muted-foreground">
                Open gaps
              </p>
              <ul className="mt-2 space-y-1">
                {panelState.openGaps.map((gap) => (
                  <li
                    key={gap.id}
