...
import { indexedDBManager, type Conversation, type Pocket } from "./indexeddb-manager.js";
...
describe("Conversation Pocket Attachment", () => {
  ...
  describe("attachPocketToConversation", () => {
    it("should successfully attach a pocket to a conversation", async () => {
      await indexedDBManager.attachPocketToConversation(testConversationId, testPocketId);

      const conversation = await indexedDBManager.getConversation(testConversationId);
    ...
    it("should update updatedAt timestamp when attaching pocket", async () => {
      const conversationBefore = await indexedDBManager.getConversation(testConversationId);
      ...
      const conversationAfter = await indexedDBManager.getConversation(testConversationId);
    ...
    it("should replace existing attached pocket when attaching a new one", async () => {
      ...
      let conversation = await indexedDBManager.getConversation(testConversationId);
      expect(conversation?.attachedPocketId).toBe(testPocketId);

      // Attach second pocket (should replace first)
      await indexedDBManager.attachPocketToConversation(testConversationId, secondPocketId);
    ...
    it("should throw error when conversation does not exist", async () => {
      ...
      await expect(
        indexedDBManager.attachPocketToConversation(nonExistentConversationId, testPocketId)
    ...
    it("should throw error when pocket does not exist", async () => {
      ...
      await expect(
        indexedDBManager.attachPocketToConversation(testConversationId, nonExistentPocketId)
      ).rejects.toThrow(/not found/i);
    });
    ...
    it("should preserve other conversation properties when attaching pocket", async () => {
      ...
      expect(conversationAfter?.tokensUsed).toBe(conversationBefore?.tokensUsed);
  ...
  describe("detachPocketFromConversation", () => {
    ...
    it("should throw error when conversation does not exist", async () => {
      ...
      await expect(
        indexedDBManager.detachPocketFromConversation(nonExistentConversationId)
      ).rejects.toThrow(/not found/i);
    });
  ...
  describe("getAttachedPocket", () => {
    it("should return attached pocket when pocket is attached", async () => {
      ...
      const pocket = await indexedDBManager.getAttachedPocket(testConversationId);
    ...
    it("should return null when conversation does not exist", async () => {
      ...
      const pocket = await indexedDBManager.getAttachedPocket(nonExistentConversationId);
  ...
  describe("getAttachedPocketId", () => {
    it("should return attached pocket ID when pocket is attached", async () => {
      ...
      const pocketId = await indexedDBManager.getAttachedPocketId(testConversationId);
    ...
    it("should return null when conversation does not exist", async () => {
      ...
      const pocketId = await indexedDBManager.getAttachedPocketId(nonExistentConversationId);
  ...
  describe("Integration scenarios", () => {
    ...
    it("should handle multiple conversations with different pocket attachments", async () => {
      ...
      try {
        ...
        await indexedDBManager.attachPocketToConversation(testConversationId, testPocketId);
        await indexedDBManager.attachPocketToConversation(secondConversationId, secondPocketId);

        // Verify each conversation has correct pocket
        const pocket1 = await indexedDBManager.getAttachedPocket(testConversationId);
        const pocket2 = await indexedDBManager.getAttachedPocket(secondConversationId);
