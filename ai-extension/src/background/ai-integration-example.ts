        ...
        initialPrompts: [
          {
            role: "system",
            content:
              "You are a helpful AI assistant for the AI Pocket extension. You help users summarize, analyze, and interact with web content.",
          },
        ],
      },
      (progress) => {
        console.log(
          `Model download progress: ${progress.percentage.toFixed(2)}%`,
        );
      },
    );
...
export async function processSimplePrompt(prompt: string): Promise<string> {
  try {
    ...
    const { defaultAISessionId } =
      await chrome.storage.local.get("defaultAISessionId");
    ...
    console.log(
      `Token usage: ${usage.used}/${usage.quota} (${usage.percentage.toFixed(2)}%)`,
    );
...
export async function monitorSessionUsage(): Promise<void> {
  ...
  const usage = aiManager.getSessionUsage(defaultAISessionId);
  console.log(
    `Session usage: ${usage.used}/${usage.quota} tokens (${usage.percentage.toFixed(2)}%)`,
  );
