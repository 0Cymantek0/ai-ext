...
import {
  ...
  CaptureErrorType,
  type CaptureOptions,
} from "../content/capture/ContentCapture.js";
import type {
  IDOMAnalyzer,
  IContentSanitizer,
  IMediaCapture,
...
function createDeps() {
  const dom: IDOMAnalyzer = {
    ...
    extractText: () => ({
      content: "Full page content with email test@example.com",
      wordCount: 6,
      characterCount: 40,
      paragraphs: [],
      headings: [],
  ...
  const media: IMediaCapture = {
    captureAllMedia: async () => ({
      ...
      audios: [{ src: "a1.mp3", duration: 3 } as any],
      videos: [],
      totalSize: 2048,
      capturedAt: Date.now(),
    }),
...
describe("ContentCapture.capture - note", () => {
  ...
  it("applies sanitization when enabled for notes", async () => {
    ...
    const res = await cc.capture({
      mode: "note",
      noteText: "Email me test@example.com",
      pocketId: "p1",
      sanitize: true,
    });
    expect(res.content.sanitization.redactionCount).toBe(1);
