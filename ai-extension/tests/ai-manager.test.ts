...
describe("AIManager", () => {
  ...
  describe("initializeGeminiNano", () => {
    ...
    it("should use custom configuration", async () => {
      const config = {
        temperature: 0.8,
        topK: 40,
        initialPrompts: [{ role: "system" as const, content: "Test prompt" }],
      };
  ...
  describe("processPrompt", () => {
    ...
    it("should pass abort signal to session", async () => {
      ...
      await aiManager.processPrompt(sessionId, "Test", {
  ...
  describe("processPromptStreaming", () => {
    ...
    it("should return a readable stream", async () => {
      ...
      const result = await aiManager.processPromptStreaming(
        sessionId,
        "Test prompt",
      );

      expect(result).toBe(mockStream);
      expect(mockSession.promptStreaming).toHaveBeenCalledWith(
        "Test prompt",
        undefined,
      );
    });
  ...
  describe("cloneSession", () => {
    ...
    it("should throw error for non-existent session", async () => {
      await expect(aiManager.cloneSession("invalid-session")).rejects.toThrow(
