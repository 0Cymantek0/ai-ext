...
import type { CapturedContent, Embedding, Pocket } from "../src/background/indexeddb-manager.js";
import type { TextChunk } from "../src/background/text-chunker.js";

/**
 * Generate mock content for testing
 */
export function createMockContent(overrides?: Partial<CapturedContent>): CapturedContent {
  return {
...
export function createSimilarEmbeddings(
  ...
  for (let i = 1; i < count; i++) {
    ...
    const vector = base.vector.map(val => 
      val * similarity + (Math.random() - 0.5) * (1 - similarity)
    );
 ...
 */
export function createMockChunks(count: number, textLength: number = 500): TextChunk[] {
 ...
 */
export function assertValidEmbedding(embedding: Embedding, expectedDimension?: number): void {
  if (!embedding.id) {
